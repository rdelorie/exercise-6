...
```{r} 
covid %>% 
  filter(date == max(date)) %>%  # Keep only the most recent date
  select(county, cases, deaths) %>%  # columns needed
  arrange(desc(cases)) %>%  # Sort by highest case count
  slice_head(n = 5)  # Get the top 5 counties

# answer: Los Angeles, New York City, Miami=Dade, Cook, Maricopa
```
...
covid %>% 
  ...
  summarize(total_cases = sum(cases, na.rm = TRUE)) %>%  # Sum cases per state
  arrange(desc(total_cases)) %>%  # Sort by highest case count
  slice_head(n = 5)  # Get the top 5 states

# answer: California, Texas, Florida, New York, Illinois
```
...
```{r}
covid %>%
  filter(date == max(date), cases > 0, county != "Unknown") %>%  # Keep most recent data & valid counties
  mutate(death_case_ratio = 100 * (deaths / cases)) %>%  # Compute death-to-case ratio as a percentage
  arrange(desc(death_case_ratio)) %>%  # Sort by highest ratio
...
```{r}
covid %>%
  filter(date == max(date), cases > 0) %>%  # Keep only the most recent data & valid cases
  group_by(state) %>%  # Group by state
  summarize(
    total_cases = sum(cases, na.rm = TRUE),  # Sum cases per state
    total_deaths = sum(deaths, na.rm = TRUE)  # Sum deaths per state
  ) %>%
  mutate(death_case_ratio = 100 * (total_deaths / total_cases)) %>%  # Compute death-to-case ratio (%)
  arrange(desc(death_case_ratio)) %>%  # Sort by highest ratio
  slice_head(n = 5)  # Select top 5 states
```
